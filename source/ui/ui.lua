...
function ui:ResizeButton(id, w, h)
	local index = self:IndexByID(id)
	self.buttons[index].w = w
	self.buttons[index].scalex = w / self.buttons[index].resource[BSTATE_DEFAULT]:getWidth()
	self.buttons[index].h = h
	self.buttons[index].scaley = h / self.buttons[index].resource[BSTATE_DEFAULT]:getHeight()
end

function ui:MoveButton(id, x, y)
	local index = self:IndexByID(id)
